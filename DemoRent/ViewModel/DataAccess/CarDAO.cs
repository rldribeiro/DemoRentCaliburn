...
namespace ViewModel.DataAccess
{
    ...
    public sealed class CarDAO
    {
        ...
        {            
            try 
            { 
                this.Cars = JsonConvert.DeserializeObject<List<Car>>(File.ReadAllText(jsonSourcePath));
                return Cars;
            }
            catch (Exception ex)
            {
        ...
        public bool UpdateCar(Car selectedCar)
        {
        ...
        /// <returns>True if update was successful</returns>
        public bool DeleteCar(Car selectedCar)
        {
